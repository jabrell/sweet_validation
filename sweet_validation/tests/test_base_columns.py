...
from sweet_validation.columns import BaseColumn, NumericBaseColumn
from sweet_validation.exceptions import DuplicatedValueError, RequiredValueError
...
def test_numeric_column():
    col = NumericBaseColumn(
        name="test", ctype=int, description="test column", items=[1, 2, 3]
    )
    assert col.ctype is int
    col = NumericBaseColumn(
        name="test", ctype=float, description="test column", items=[1, 2, 3]
    )
    assert col.ctype is float

    # only numeric types are allowed
    with pytest.raises(ValidationError):
        col = NumericBaseColumn(
            name="test", ctype=str, description="test column", items=[1, 2, 3]
        )
